...
namespace Api.Service.Test.AutoMapper
{
    public class CepMapper : BaseTesteService   
    {
        ...
        public void E_Possivel_Mapear_os_Cep()
        {
            var model = new CepModel
            {
                Id = Guid.NewGuid(),
                Cep = Faker.RandomNumber.Next(1, 100000).ToString(),
...

            {
                ...
                var cepUpdateResult = Mapper.Map<CepDtoUpdateResult>(entity);
                Assert.Equal(cepUpdateResult.Id, entity.Id);
                Assert.Equal(cepUpdateResult.Cep, entity.Cep);
                Assert.Equal(cepUpdateResult.Logradouro, entity.Logradouro);
                Assert.Equal(cepUpdateResult.Numero, entity.Numero);
                Assert.Equal(cepUpdateResult.UpdateAt, entity.UpdateAt);
